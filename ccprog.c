...
#define SET_DD_INPUT()      (pinMode(gpio_dd, INPUT))
//! Set programmer DD line as output
#define SET_DD_OUTPUT()     (pinMode(gpio_dd, OUTPUT))
...
void delayWrite(int pin, int val) {
  digitalWrite(pin, val);
  delay_ns(100);
}

int delayRead(int pin) {
  delay_ns(100);
...
void write_debug_byte(unsigned char data)
{
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    delayWrite(gpio_dd, (data & 0x80) ? 1 : 0);
    data <<= 1;
    delayWrite(gpio_dc, 0); // set clock low (DUP capture flank)
  }
}
...
unsigned char read_debug_byte(void)
{
  ...
  for (i = 0; i < 8; i++)
  {
    delayWrite(gpio_dc, 1);
    data <<= 1;
    data |= delayRead(gpio_dd);  // Read DD line
    delayWrite(gpio_dc, 0);
  }
  return data;
}
...
{
  int i;
  delayWrite(gpio_rst, 0);
  usleep(5000);
  delayWrite(gpio_rst, 1);
  usleep(10);
}
...
int debug_init(unsigned char *chip_id, unsigned char *revision)
{
  ...
  delayWrite(gpio_rst, 1); // RESET_N high
...
void close_gpios()
{
  pinMode(gpio_dc, INPUT);
  pinMode(gpio_dd, INPUT);
  pinMode(gpio_rst, INPUT);
}

#define TOTAL_FLASH_SIZE 0x8000
...
int read_flash(char *filename)
{
  ...
  if (fp != NULL) {
    for (int i = 0; i < TOTAL_FLASH_SIZE / 1024; i++) {
      ok = read_flash_memory_block(i * 1024, 1024, buf + i * 1024);
      if (!ok) {
        printf("Read flash failed!\n");
        return -1;
      }
    ...
    fwrite(buf, 1, TOTAL_FLASH_SIZE, fp);
    fclose(fp);
  } else {
...
void init_gpios(int dc, int dd, int reset_n) {
  ...
  pinMode(gpio_dd, OUTPUT);
  pullUpDnControl(gpio_dd, PUD_UP);

  // Init levels on gpios
  digitalWrite(gpio_rst, 1);
  digitalWrite(gpio_dc, 0);
  digitalWrite(gpio_dd, 0);
}
...
void usage(char *prog_name, char *command) {
  if (command) {
    if (strcmp(command, "write") == 0) {
      ...
      printf("Usage: %s read filename.bin\n\n", prog_name);
    }
  } else {
    ...
    printf("   -p DC,DD,RESET              specify wiringPi pins for debugging cc chip:\n\n");
  }
  exit(1);
}
